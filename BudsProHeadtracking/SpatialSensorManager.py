...
class SpatialSensorManager:
    def __init__(self, socket, data_callback, verbose=False, debug=False):
        ...
        self.service = BluetoothService(
            socket, self.__onMessageReceived, debug)
        self.timer = None

    def attach(self):
        self.service.sendPacket(self.MSG_SPATIAL_AUDIO_ENABLE, bytes(1))
        self.service.sendPacket(
            self.MSG_SPATIAL_AUDIO_CONTROL, bytes(self.CID_ATTACH))
        self.timer = RepeatedTimer(2, self.__keepAlive)

    def detach(self):
        self.service.sendPacket(
            self.MSG_SPATIAL_AUDIO_CONTROL, bytes(self.CID_DETACH))
        self.service.sendPacket(self.MSG_SPATIAL_AUDIO_ENABLE, bytes(0))
        self.timer.stop()

    def __keepAlive(self):
        self.service.sendPacket(
            self.MSG_SPATIAL_AUDIO_CONTROL, bytes(self.CID_KEEP_ALIVE))
    ...
    def __onMessageReceived(self, data):
        if data[3] == self.MSG_SPATIAL_AUDIO_CONTROL:
            ...
            if event == self.DID_SENSOR_STUCK and self.verbose:
                ...
                if len(input) == 8:
                    quaternion = [
                        one/10000.0 for one in struct.unpack('hhhh', payload[:-1])]
